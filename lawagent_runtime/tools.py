...
from .evidence_views import CaseEvidenceView, LawEvidenceView
from .messages import AgentRole
from .patches import PatchOperation, PatchTarget, StatePatch
from .pii import PIIPolicy
from .state import AttemptStatus, EvidenceRelation, SourceType, new_id
...
def tool_result_to_state_patches(
    result: ToolResult,
    *,
    run_id: str,
    author_role: AgentRole = AgentRole.RETRIEVAL,
    ...
    attempt_payload = {
        ...
        "normalized_query": str(result.metadata.get("normalized_query") or ""),
        "applied_filters": result.metadata.get("applied_filters") if isinstance(result.metadata.get("applied_filters"), dict) else {},
        "top_k": int(result.metadata.get("top_k") or max(len(result.items), 1)),
        "result_count": len(result.items),
        "latency_ms": result.latency_ms,
        "status": _attempt_status(result.status).value,
        "error_code": result.metadata.get("error_code") if isinstance(result.metadata.get("error_code"), str) else result.error,
    ...
    patches = [
        StatePatch(
            ...
            target=PatchTarget.RETRIEVAL_ATTEMPTS,
            operation=PatchOperation.APPEND,
            payload=attempt_payload,
            reason=f"tool_result:{result.tool_name}:{result.status.value}",
        )
    ]
        ...
        for item in result.items:
            patches.append(
                StatePatch(
                    ...
                    operation=PatchOperation.APPEND,
                    payload=_evidence_payload(item, issue_id),
                    reason=f"tool_evidence:{result.tool_name}",
                )
            )
    return patches
...
def _attempt_status(status: ToolResultStatus) -> AttemptStatus:
    return {
        ToolResultStatus.SUCCESS: AttemptStatus.SUCCESS,
        ToolResultStatus.PARTIAL: AttemptStatus.PARTIAL,
        ToolResultStatus.EMPTY: AttemptStatus.EMPTY,
        ToolResultStatus.BLOCKED: AttemptStatus.FAILED,
        ToolResultStatus.FAILED: AttemptStatus.FAILED,
    }[status]


def _evidence_payload(item: CaseEvidenceView | LawEvidenceView, issue_id: str | None) -> dict[str, Any]:
    issue_ids = [issue_id] if issue_id else []
    if isinstance(item, CaseEvidenceView):
        snippet = "\n".join(
            value for value in (item.fact_snippet, item.reasoning_snippet, item.result_summary) if value
        )
        return {
            "evidence_id": f"case:{item.case_id}",
            ...
            "score": item.score,
            "issue_ids": issue_ids,
            "supports": EvidenceRelation.UNCLEAR.value,
            "warnings": item.warnings,
        }
    article = f"第{item.article_no}条" if item.article_no else ""
    return {
        "evidence_id": f"law:{item.chunk_id}",
        "source_type": SourceType.STATUTE.value,
        "source_id": item.chunk_id,
        "parent_id": item.law_version_id,
        "title": item.title,
        "citation_label": f"{item.title}{article}",
